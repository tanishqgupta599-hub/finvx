  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="text-sm font-medium">Top actions</div>
        <div className="text-xs text-zinc-500">{overwhelmMode ? "Overwhelm Mode on • 1 gentle step" : "3 focused suggestions"}</div>
      </div>
      <div className="grid gap-3 md:grid-cols-3">
        {filtered.map((action) => (
          <Card key={action.id} className="bg-gradient-to-br from-white to-zinc-50 dark:from-zinc-950 dark:to-zinc-900">
            <CardContent>
              <div className="flex items-start justify-between gap-2">
                ...
                <span className="rounded-full bg-zinc-900 px-2 py-0.5 text-[10px] font-medium text-white dark:bg-zinc-100 dark:text-black">
                  {action.impactLabel}
                </span>
              </div>
              <div className="mt-3 flex items-center justify-between gap-2">
                <Link
                  ...
                  className="inline-flex h-8 flex-1 items-center justify-center rounded-2xl bg-zinc-900 px-3 text-xs font-medium text-white transition-colors hover:bg-zinc-800 dark:bg-zinc-100 dark:text-black dark:hover:bg-zinc-200"
                >
                  View steps
                </Link>
                <button
